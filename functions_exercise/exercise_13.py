"""
Ćwiczenie 14
Zaimplementuj funkcję o nazwie sort_by_length(),
która będzie sortować listę słów według ich długości.
Funkcja powinna przyjmować listę słów i zwracać nową
posortowaną listę. Do sortowania użyj funkcji wbudowanej
sorted() oraz wyrażenia lambda do określenia klucza sortowania.
Przykłady użycia:
...
def sort_by_length(word):
    return sorted(word, key=lambda x: len(x))
