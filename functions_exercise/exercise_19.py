"""
Ćwiczenie 20
Zaimplementuj funkcję o nazwie person_info(), która będzie zwracać
opis osoby na podstawie przekazanych do niej argumentów typu
klucz-wartość według poniższego schematu:
f'{name} is {age} years old and lives in {city}.'
Funkcja powinna działać dla dowolnej liczby argumentów typu
klucz-wartość. Do tego użyj składni **kwargs, która pozwala
na przekazywanie zmiennej liczby argumentów typu klucz-wartość
do funkcji. Jeśli argument name nie zostanie przekazany ustaw
domyślną wartość 'John'. Jeśli jakiś inne argumenty nie zostaną
przekazane przy wywołaniu funkcji wstaw wartość <unknown>.
Przykłady użycia:
